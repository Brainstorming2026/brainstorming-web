...
export function InternalLeadEmail({ nombre, email, telefono, empresa, guideTitle }: InternalLeadEmailProps) {
  const fields = [
    { label: 'Nombre', value: nombre },
    { label: 'Email', value: email },
    telefono && { label: 'Teléfono', value: telefono },
    empresa && { label: 'Empresa', value: empresa },
    { label: 'Guía', value: guideTitle },
  ].filter((f): f is { label: string, value: string } => Boolean(f))

  return (
    <EmailLayout preview={`Nuevo lead — Guía: ${nombre}`}>
