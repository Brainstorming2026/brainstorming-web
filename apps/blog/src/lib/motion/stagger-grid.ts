...
export function staggerGrid(items: Element[] | NodeListOf<Element>, options: StaggerGridOptions = {}) {
  ...
  const from: gsap.TweenVars
    = variant === 'wipe'
      ? { clipPath: 'inset(0% 0% 100% 0%)', opacity: 0, y: 44, scale: 0.96, rotate: rotateFrom }
      : { opacity: 0, scale: 0.9, y: () => gsap.utils.random(18, 44) }

  const to: gsap.TweenVars
    = variant === 'wipe'
      ? { clipPath: 'inset(0% 0% 0% 0%)', opacity: 1, y: 0, scale: 1, rotate: 0, duration: itemDuration, ease: ease.out }
